...
//send controller changes to these ctlins
int controllerA1 = 10;
int controllerA2 = 11;
int controllerA3 = 12;
int controllerA4 = 13;
...
if (button0.fallingEdge()) {
  ...
  envelope1.noteOn();
  bowing = true;
  usbMIDI.sendControlChange(controllerA1, 1, channel);
}
...
while((micros()-timer0)<10000){ // this delays for the remainder of the time up to 10ms
 ...
 usbMIDI.sendControlChange(controllerA3, L_nut, channel);
  usbMIDI.sendControlChange(controllerA4, L_between, channel);    
  usbMIDI.sendControlChange(controllerA5, bow_angle, channel);
}
...
void pingpong() {
  hue = random16(255);
  for(int i=0; i<n_NUM_LEDS-bouncewidth; i++) {
    trailhue = hue;
    FastLED.clear();
    for(int j=0; j<bouncewidth; j++) {
      n_leds[i+j] = CHSV(trailhue, 255, 255);
      n_leds[i-j].fadeToBlackBy(50);
      trailhue = trailhue + 5;
    }
    FastLED.show();
    delay(deeelay);
  }
  deeelay = deeelay - delaychange;
  delaychange = delaychange + 1;
  ...
  for(int i=0; i<n_NUM_LEDS-bouncewidth; i++) {
    ...
    trailhue = hue; 
    for(int j=0; j<bouncewidth; j++) {
      n_leds[(n_NUM_LEDS-i-j)] = CHSV(trailhue, 255, 255);
      // n_leds[(n_NUM_LEDS-i+j)].fadeToBlackBy(50);
      trailhue = trailhue + 5;
    }
    FastLED.show();
    delay(deeelay);
